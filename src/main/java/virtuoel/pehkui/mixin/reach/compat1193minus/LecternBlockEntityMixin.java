...
public abstract class LecternBlockEntityMixin
{
	...
	@Dynamic
	@ModifyExpressionValue(method = MixinConstants.CAN_PLAYER_USE, at = @At(value = "CONSTANT", args = "doubleValue=0.5D", ordinal = 0))
	private double pehkui$canPlayerUse$xOffset(double value, Player player)
	{
		return ScaleUtils.getBlockXOffset(field_17391.getBlockPos(), player);
	}
	
	@Dynamic
	@ModifyExpressionValue(method = MixinConstants.CAN_PLAYER_USE, at = @At(value = "CONSTANT", args = "doubleValue=0.5D", ordinal = 1))
	private double pehkui$canPlayerUse$yOffset(double value, Player player)
	{
		return ScaleUtils.getBlockYOffset(field_17391.getBlockPos(), player);
	}
	
	@Dynamic
	@ModifyExpressionValue(method = MixinConstants.CAN_PLAYER_USE, at = @At(value = "CONSTANT", args = "doubleValue=0.5D", ordinal = 2))
	private double pehkui$canPlayerUse$zOffset(double value, Player player)
	{
		return ScaleUtils.getBlockZOffset(field_17391.getBlockPos(), player);
	}
